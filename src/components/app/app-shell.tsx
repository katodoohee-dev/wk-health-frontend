...
import { BookOpen, Grid3x3, Home, Loader2, Route as RouteIcon, ScanLine } from "lucide-react";
...
import { RouteMenu } from "@/components/app/route-menu";

const tabs = [
  { to: "/", label: "หน้าแรก", icon: Home },
  { to: "/diary", label: "ไดอารี", icon: BookOpen },
  { to: "/run", label: "วิ่ง", icon: RouteIcon },
] as const;
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  {!isAuthRoute && isAuthenticated && <><VoiceControl profileName={me.data?.["name"] as string | undefined} bodyWeightKg={Number(me.data?.["weightKg"] ?? 60)} onExercise={(result) => window.dispatchEvent(new CustomEvent("wk:voice-exercise", { detail: result }))} onStartGps={async () => { const ok = await gpsBridge.start(); if (ok) window.dispatchEvent(new CustomEvent("wk:gps-started")); if (!ok) void navigate({ to: "/pedometer" }); }} onStopGps={async () => { await gpsBridge.stop(); }} onOpenProfileModal={() => void navigate({ to: "/profile" })} /><NavigationOverlayLoader /><AppCommandRuntimeLoader /><RouteMenu open={menuOpen} onClose={() => setMenuOpen(false)} pathname={pathname} /></>}
  {!isAuthRoute && isAuthenticated && <nav className="fixed inset-x-0 bottom-0 z-40 pb-[env(safe-area-inset-bottom)]"><MiniPlayer /><div className="mx-auto mb-3 w-full max-w-md px-4"><div className="hairline-t relative grid grid-cols-5 items-center rounded-[1.75rem] border border-hairline bg-background/90 px-2 py-2 backdrop-blur-xl">{tabs.slice(0,2).map((t) => <NavItem key={t.to} {...t} active={pathname === t.to} />)}<div className="relative grid place-items-center"><Link to="/scan" aria-label="สแกนอาหาร" className="press absolute -top-9 grid size-16 place-items-center rounded-[1.35rem] bg-foreground text-background"><ScanLine className="size-7" /></Link><span className="mt-6 text-[10px] font-medium text-muted-foreground">สแกน</span></div>{tabs.slice(2).map((t) => <NavItem key={t.to} {...t} active={pathname === t.to} />)}<button type="button" onClick={() => setMenuOpen(true)} className="press flex flex-col items-center gap-1 rounded-2xl py-2 text-[10px] font-medium text-muted-foreground"><span className="grid size-9 place-items-center rounded-xl"><Grid3x3 className="size-5" /></span><span className="label-editorial truncate text-inherit">เมนู</span></button></div></div></nav>}</div></MusicProvider>;
}
