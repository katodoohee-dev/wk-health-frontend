  ...
  return (
    <AppShell eyebrow="Sound system" title="Sound control" wide>
      <PageHeader
        ...
        description="The sound system is restored to the production frontend and writes directly to the existing backend sound endpoint."
      ...
      <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
        <Panel className="rounded-none shadow-none">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => void save({ ...settings, voiceEnabled: !settings.voiceEnabled })} className={`sound-toggle ${settings.voiceEnabled ? 'is-active' : ''}`} aria-pressed={Boolean(settings.voiceEnabled)}>
              <span><b>Voice output</b><small>{settings.voiceEnabled ? 'Enabled' : 'Muted'}</small></span><i />
            </button>
        ...
        <aside className="space-y-6">
          ...
          {error && <Panel className="rounded-none border-destructive/40 shadow-none"><p className="eyebrow">ERROR</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{error}</p></Panel>}
        </aside>
      </div>
    </AppShell>
  );
