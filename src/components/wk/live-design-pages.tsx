...
import '@/components/wk-design.css';
...
export function LiveStatsDesign() {
  ...
  const values = rows.length ? rows.slice(-12).map(row => n(pick(row, 'calories', 'kcal', 'burn', 'steps'))) : [30,42,36,54,47,63,55,66,62,72,68,82];
  const max = Math.max(...values, 1);
  return <AppShell eyebrow="Analysis · 12 weeks" title="The long line, not the spike.">
    ...
    <Panel className="mt-6"><SectionHeader eyebrow="Live weekly trend" title="Active load"/><div className="mt-8 flex h-48 items-end gap-3">{values.map((value, index) => <i key={index} className="flex-1 bg-foreground" style={{height:`${Math.max(5,(value/max)*100)}%`}} title={String(value)}/>)}</div></Panel>
  </AppShell>;
}
