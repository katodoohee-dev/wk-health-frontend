  ...
  return (
    <div className={cn("panel grain overflow-hidden p-0", className)}>
      <div className="flex items-center gap-3 p-3">
        <div
          className="grid size-11 shrink-0 place-items-center rounded-md border border-border bg-surface-2"
          aria-hidden="true"
        >
          <div className="flex items-end gap-[2px]">
            {[8, 14, 6, 11].map((h, i) => (
              <span
                key={i}
                className={cn("w-[2px] rounded-full bg-foreground", playing && "animate-pulse")}
      ...
      <div className="flex items-center gap-3 px-3 pb-3">
        <span className="numeric text-[0.625rem] text-muted-foreground">{fmt(track.elapsed)}</span>
        <div
        ...
        <span className="numeric text-[0.625rem] text-muted-foreground">{fmt(track.duration)}</span>
      </div>
    </div>
  );
}
