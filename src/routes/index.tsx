...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "ศูนย์กลางสุขภาพรายวันของ WK Health" },
      { property: "og:title", content: "WK Health — Health OS" },
      { property: "og:description", content: "ข้อมูลสุขภาพจริง พร้อมสแกนอาหาร วิ่ง GPS และผู้ช่วย AI" },
    ],
  }),
  component: Home,
});
...
const tools = [
  ...
  { to: "/notifications", icon: BellRing, title: "การแจ้งเตือน", desc: "แจ้งเตือนอัจฉริยะ", tint: "bg-mint-soft text-mint" },
] as const;
...
function Home() {
  ...
  return (
    <div className="rise-in pb-8">
      ...
      {checkin.data && (
        <button onClick={() => !checkin.data!.alreadyCheckedInToday && doCheckin.mutate()} disabled={checkin.data.alreadyCheckedInToday || doCheckin.isPending} className="press glass-strong mb-4 flex w-full items-center gap-3 rounded-3xl p-4 text-left shadow-soft disabled:cursor-default">
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-2"><span className="font-display font-semibold">Streak {checkin.data.streak} วัน</span>{checkin.data.freezeAvailable > 0 && <span className="flex items-center gap-0.5 text-xs text-sky"><Snowflake className="size-3" />×{checkin.data.freezeAvailable}</span>}</span>
            <span className="block truncate text-xs text-muted-foreground">{checkin.data.greeting}</span>
          </span>
          {!checkin.data.alreadyCheckedInToday && <span className="bg-mint-gradient shrink-0 rounded-2xl px-3 py-2 text-xs font-medium text-primary-foreground shadow-glow">{doCheckin.isPending ? "กำลังเช็คอิน…" : "เช็คอิน"}</span>}
        </button>
      )}

      {stats.isLoading ? <Skeleton className="h-64 w-full rounded-3xl" /> : stats.isError ? <ErrorState error={stats.error} onRetry={() => void stats.refetch()} /> : (
        <section className="glass-strong rounded-3xl p-5 shadow-soft">
          <div className="grid gap-5 sm:grid-cols-[150px_1fr] sm:items-center">
            <div className="mx-auto grid size-36 place-items-center rounded-full border-[10px] border-surface-2 border-t-foreground shadow-glow">
              <div className="text-center"><p className="font-display text-3xl font-bold tabular-nums">{remaining}</p><p className="text-xs text-muted-foreground">kcal เหลือได้</p></div>
            </div>
            <div className="min-w-0 space-y-3">
              ...
              <Bar label="โปรตีน" value={s!.protein} max={s!.proteinGoal} />
              <Bar label="คาร์บ" value={s!.carb} max={s!.carbGoal} />
              <Bar label="ไขมัน" value={s!.fat} max={s!.fatGoal} />
            </div>
          </div>
        </section>
      )}
      ...
      <section className="mt-6">
        <div className="mb-3 flex items-center justify-between"><h2 className="font-display text-lg font-semibold">เครื่องมือของคุณ</h2><Link to="/scan" className="text-xs text-muted-foreground">สแกนอาหาร</Link></div>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {tools.map((t) => <Link key={t.to} to={t.to} className="press glass-strong group rounded-3xl p-4 shadow-soft"><span className={`grid size-11 place-items-center rounded-2xl ${t.tint}`}><t.icon className="size-5" /></span><p className="mt-3 truncate font-display font-semibold">{t.title}</p><p className="truncate text-xs text-muted-foreground">{t.desc}</p></Link>)}
        </div>
      </section>
      ...
      <section className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="font-display text-lg font-semibold">มื้อล่าสุด</h2><Link to="/diary" className="text-xs text-primary">ดูทั้งหมด</Link></div>
        {diary.isLoading ? <div className="space-y-2"><Skeleton className="h-16 w-full rounded-2xl" /><Skeleton className="h-16 w-full rounded-2xl" /></div> : diary.isError ? <ErrorState error={diary.error} onRetry={() => void diary.refetch()} /> : diary.data!.length === 0 ? <p className="glass-strong rounded-3xl p-6 text-center text-sm text-muted-foreground">ยังไม่มีมื้ออาหารวันนี้ — เริ่มจากการสแกนอาหารได้เลย</p> : <div className="space-y-2">{diary.data!.slice(-3).reverse().map((m) => <div key={m.id} className="glass-strong flex items-center gap-3 rounded-2xl p-3 shadow-soft"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-muted text-xl">{m.emoji}</span><div className="min-w-0 flex-1"><p className="truncate font-medium">{m.name}</p><p className="truncate text-xs text-muted-foreground">{m.slot} · {m.time}</p></div><span className="shrink-0 text-sm font-semibold tabular-nums">{m.kcal} kcal</span></div>)}</div>}
      </section>
    </div>
  );
}
...
function MiniStat({ label, value, icon }: { label: string; value: number; icon: string }) {
  return <div className="glass rounded-2xl p-3"><div className="text-base">{icon}</div><div className="mt-1 font-display font-semibold tabular-nums">{Math.round(value)}</div><div className="text-[10px] text-muted-foreground">{label}</div></div>;
}

function Bar({ label, value, max }: { label: string; value: number; max: number }) {
  const pct = max ? Math.min(100, Math.max(0, (value / max) * 100)) : 0;
  return <div><div className="mb-1 flex items-center justify-between text-xs"><span>{label}</span><span className="text-muted-foreground">{Math.round(value)} / {Math.round(max)} g</span></div><div className="h-1.5 overflow-hidden rounded-full bg-surface-2"><div className="h-full rounded-full bg-foreground transition-all" style={{ width: `${pct}%` }} /></div></div>;
}
