...
// ใช้ backend ที่กำหนดผ่าน env ก่อน และ fallback ไป same-origin /api ของ Render
...
export async function apiFetch<T = Json>(path: string, options: { method?: string; body?: unknown; signal?: AbortSignal } = {}): Promise<T> {
  const candidates = Array.from(new Set([
    `${API_BASE_URL}${path}`,
    ...(typeof window !== "undefined" && window.location.origin !== API_BASE_URL && !isLocal ? [`${window.location.origin}${path}`] : []),
  ]));
  const token = getToken();
  for (const url of candidates) {
    let res: Response;
    try {
      res = await fetch(url, { method: options.method ?? "GET", headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) }, ...(options.body !== undefined ? { body: JSON.stringify(options.body) } : {}), ...(options.signal ? { signal: options.signal } : {}) });
    } catch {
      if (url !== candidates[candidates.length - 1]) continue;
      throw new ApiError("เชื่อมต่อ WK Health Backend ไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตหรือเซิร์ฟเวอร์");
    }
    let data: Json = {};
    try { data = (await res.json()) as Json; } catch { if (!res.ok) { if ([502,503,504].includes(res.status) && url !== candidates[candidates.length - 1]) continue; throw new ApiError(`เกิดข้อผิดพลาด (${res.status})`, res.status); } }
    if (!res.ok || data["success"] === false) {
      if ([502,503,504].includes(res.status) && url !== candidates[candidates.length - 1]) continue;
      const msg = (typeof data["error"] === "string" && data["error"]) || (typeof data["message"] === "string" && data["message"]) || `เกิดข้อผิดพลาด (${res.status})`;
      throw new ApiError(msg, res.status);
    }
    return data as T;
  }
  throw new ApiError("เชื่อมต่อ WK Health Backend ไม่สำเร็จ");
}
...
export async function apiLogin(email: string, password: string) { return readAuth(await apiFetch("/api/auth/login", { method: "POST", body: { email: email.trim().toLowerCase(), password } })); }
export async function apiRegister(input: { name: string; email: string; password: string }) { return readAuth(await apiFetch("/api/auth/register", { method: "POST", body: { email: input.email.trim().toLowerCase(), password: input.password, displayName: input.name.trim() } })); }
export async function apiLogout() { try { await apiFetch("/api/auth/logout", { method: "POST" }); } catch {} }
...
export async function apiMoodList(): Promise<MoodOption[]> { const data = await apiFetch("/api/mood/list"); const list = pick<Json[]>(data, ["moods", "items", "data", "list"], []); return (Array.isArray(list) ? list : []).map((m) => ({ key: String(pick(m, ["mood_id", "key", "id", "value", "mood", "name"], "")), label: pick<string>(m, ["name_th", "label", "name", "title"], ""), emoji: pick<string>(m, ["icon", "emoji"], "🙂"), hint: pick<string>(m, ["hint", "description", "desc"], "") })); }
export async function apiMoodRecommend(mood: string, meal: "breakfast" | "main") { const data = await apiFetch(`/api/mood/recommend?mood=${encodeURIComponent(mood)}&meal=${meal}`); const list = pick<Json[]>(data, ["recommendations", "menus", "items", "data"], []); return (Array.isArray(list) ? list : []).map<MoodMenu>((m) => ({ name: pick<string>(m, ["name", "menu", "title"], "-"), kcal: num(pick(m, ["kcal", "calories"], 0)), emoji: pick<string>(m, ["emoji", "icon"], "🍽️"), why: pick<string>(m, ["coach_tip", "why", "reason", "benefit", "description"], "") })); }
