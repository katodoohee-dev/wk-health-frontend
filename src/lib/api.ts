...
/**
 * FIX: backend คืน user เป็น snake_case ล้วน (display_name, weight_kg, height_cm, goal_kcal)
 * แต่หน้า profile.tsx อ่านแบบ camelCase (user.name, user.weightKg, user.heightCm) ตรงๆ
 * ไม่มี fallback เลย ทำให้ช่องชื่อ/น้ำหนัก/ส่วนสูงว่างเปล่าเสมอตอนโหลดหน้า
 * เลย normalize ให้เป็น camelCase ที่นี่ที่เดียว ก่อนส่งต่อให้ทุกหน้าใช้
 */
...
/**
 * FIX x2:
 * 1) หน้า profile.tsx ส่ง key "name" แต่ backend zod schema (PATCH /api/auth/me) รับเฉพาะ "displayName"
 *    key ที่ไม่รู้จักจะถูก zod ทิ้งเงียบๆ (ไม่ error) ทำให้กด "บันทึกโปรไฟล์" แล้วชื่อไม่เคยอัปเดตจริง
 * 2) backend PATCH ตอบแค่ {success:true} ไม่มี user กลับมา ของเดิม pick หา user แล้วได้ null เสมอ
 *    ทำให้ setUser(null) เงียบๆ ต้องรีเฟรชหน้าถึงจะเห็นค่าที่บันทึกจริง — เลย re-fetch /me ต่อให้เอง
 */
...
function readDiaryItem(raw: Json): DiaryItem {
  return {
    ...
    name: pick<string>(raw, ["food_name", "name", "food", "title", "description"], "ไม่ระบุชื่อ"),
...
export async function apiStatsToday(): Promise<TodayStats> {
  ...
  const s = pick<Json>(data, ["totals", "stats", "today", "data"], data);
  return {
...
/**
 * FIX: บั๊กใหญ่ 🔴 — backend (POST /api/budget/plan) ตอบกลับ
 * { success, tier, plan: [{ breakfast, lunch, dinner, dayCost }, ...] }
 * คือ "plan" เป็น array ของแต่ละวันตรงๆ อยู่แล้ว และแต่ละวันมี breakfast/lunch/dinner
 * เป็น object มื้อแยกกัน 3 key — ไม่ได้ห่อด้วย {days:[...]} และไม่มี key "meals" เป็น array เลย
 * ของเดิมพยายามหา p.days ซ้อนอีกชั้น (p คือ array จาก data.plan) ทำให้ rawDays ว่างเปล่าตลอด
 * (pick() เจอ array ที่ไม่มี key "days"/"plan"/"items" เป็น property จริงๆ เลย fallback เป็น [])
 */
export async function apiBudgetPlan(input: BudgetPlanInput): Promise<BudgetPlan> {
  ...
  const days: BudgetDay[] = (Array.isArray(rawDays) ? rawDays : []).map((d, i) => {
    const meals: BudgetMeal[] = (["breakfast", "lunch", "dinner"] as const)
      .map((slot) => {
        const m = pick<Json | null>(d, [slot], null);
        if (!m) return null;
        return {
          name: pick<string>(m, ["name", "menu", "title"], "-"),
          price: num(pick(m, ["cost", "price", "baht"], 0)),
          kcal: num(pick(m, ["kcal", "calories"], 0)),
    ...
    return {
      day: `วันที่ ${i + 1}`,
      meals,
      total: num(pick(d, ["dayCost", "total", "cost"], meals.reduce((s, m) => s + m.price, 0))),
    };
  });
  return {
    days,
    totalCost: num(
      pick(data, ["totalCost", "total", "cost"], days.reduce((s, d) => s + d.total, 0)),
    ),
    note: pick<string>(data, ["note", "advice", "summary"], ""),
  };
}
...
export async function apiPedometerToday(): Promise<PedometerToday> {
  ...
  return {
    ...
    distanceKm: num(pick(p, ["distance_km", "distanceKm", "distance", "km"], 0)),
    kcal: num(pick(p, ["kcal_burned", "kcal", "calories", "burned"], 0)),
...
/**
 * FIX: บั๊กใหญ่ 🔴 — backend (POST /api/workout/plan) ตอบกลับ
 * { success, plan: [{ day, exercises: [...] }] } คือ "plan" เป็น array ของแต่ละวันตรงๆ
 * ของเดิมดึง p = data.plan (ได้ array) แล้วพยายามหา p.days ซ้อนอีกชั้น ซึ่งไม่มีจริง
 * ทำให้ rawDays ว่างเปล่าตลอด (เหมือนบั๊กเดียวกับ apiBudgetPlan ด้านบน)
 */
...
/**
 * FIX: บั๊กใหญ่ 🔴 — backend (GET /api/workout/history) ตอบกลับ { success, workouts: [...] }
 * key "workouts" ไม่เคยอยู่ใน pick list เดิมเลย ทำให้ list ว่างเปล่าตลอด (ประวัติไม่ขึ้นเลย)
 * และคอลัมน์จริงในตารางเป็น snake_case: exercise_name, kcal_burned
 */
export async function apiWorkoutHistory(): Promise<WorkoutHistoryItem[]> {
  const data = await apiFetch("/api/workout/history");
  const list = pick<Json[]>(data, ["workouts", "history", "items", "logs", "data"], []);
  return (Array.isArray(list) ? list : []).map((h) => ({
    id: String(pick(h, ["id", "_id"], Math.random().toString(36).slice(2))),
    name: pick<string>(h, ["exercise_name", "exerciseName", "name", "exercise"], "-"),
    minutes: num(pick(h, ["minutes", "duration"], 0)),
    kcal: num(pick(h, ["kcal_burned", "kcal", "calories", "burn", "burned"], 0)),
...
export async function apiRouteStop(input: {
  ...
  return {
    ...
    kcal: num(pick(r, ["kcalBurned", "kcal", "calories", "burned"], 0)),
...
export async function apiRouteHistory(): Promise<RouteHistoryItem[]> {
  ...
  return (Array.isArray(list) ? list : []).map((r) => ({
    ...
    distanceKm: num(pick(r, ["distance_km", "distanceKm", "distance", "km"], 0)),
    kcal: num(pick(r, ["kcal_burned", "kcal", "calories", "burned"], 0)),
    durationSeconds: num(pick(r, ["duration_seconds", "durationSeconds", "duration"], 0)),
    date: String(pick(r, ["ended_at", "date", "createdAt", "created_at", "endedAt"], ""))
      .slice(0, 16)
      .replace("T", " "),
  }));
}
...
export async function apiMusicHistory(): Promise<PlayHistoryItem[]> {
  ...
  return (Array.isArray(list) ? list : []).map((h) => ({
    ...
    playedAt: String(pick(h, ["played_at", "playedAt", "createdAt", "created_at", "date"], ""))
      .slice(0, 16)
      .replace("T", " "),
  }));
}
...
/**
 * FIX: บั๊กใหญ่ 🔴 — backend (POST /api/music/played) zod schema ต้องการ {url, title?, type}
 * ของเดิมส่ง {trackId} ไม่ตรง schema เลย ได้ 400 ทุกครั้งที่กดเล่นเพลง (ประวัติการฟังเลยไม่เคยถูกบันทึก)
 * เปลี่ยนให้รับ track ทั้งก้อนแล้วส่งฟิลด์ที่ backend ต้องการจริงแทน
 */
export async function apiMusicPlayed(track: Pick<Track, "url" | "title" | "type">) {
  return apiFetch("/api/music/played", {
    method: "POST",
    body: { url: track.url, title: track.title, type: track.type },
  });
}
