...
const root = await firstExisting([path.join(projectRoot,'dist'),path.join(projectRoot,'.output','public')]);
...
const server=http.createServer(async(req,res)=>{
  ...
  if((req.url||'/').split('?')[0]==='/healthz'){res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});return res.end(JSON.stringify({status:'ok',service:'wk-health-frontend'}));}
...
server.listen(port,'0.0.0.0',()=>console.log(`WK Health frontend listening on ${port}; build root: ${root}`));
